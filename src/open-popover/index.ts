(() => {
  ...
  type SignificantPolicy = {
    test: (significant: string) => boolean,
    minutes: number | ((significant: string) => number),
    subtractFromLackTime: boolean,
  }

  const significantPolicies: SignificantPolicy[] = [
    {
      test: (significant: string) => (
    ...
    {
      test: (significant: string) => /(반차|건강검진|백신접종|기타\(반일\))/.test(significant),
      minutes: HALF_WORKING_HOURS_PER_DAY * MINUTES_PER_HOUR,
      subtractFromLackTime: true,
    },
    {
      test: (significant: string) => significant.startsWith('출장'),
      minutes: (significant: string) => getBusinessTripMinutes(significant),
      subtractFromLackTime: false,
    },
  ]

  const significantPolicyOrNull = (significant: string): SignificantPolicy | null => {
    return significantPolicies.find((policy) => policy.test(significant)) ?? null
  }

  const convertSignificantToMins = (significant: string): number => {
  ...
  const shouldSubtractSignificantFromLackTime = (significant: string): boolean => {
    return significantPolicyOrNull(significant)?.subtractFromLackTime ?? false
  }

  const getBusinessTripMinutes = (text: string): number => {
  ...
  const endOfMonth = (date = new Date()): Date => {
  ...
  const parseDateText = (text: string | undefined): Date | null => {
    if (!text) {
      return null
    }
  ...
  const childElementText = (tr: HTMLTableRowElement, index: number): string => {
    const child = tr.childNodes[index]
    return child instanceof HTMLElement ? child.innerText : ''
  }

  const tableRows = (table: HTMLDivElement | null): HTMLTableRowElement[] => {
    return [...(table?.querySelector('tbody')?.querySelectorAll<HTMLTableRowElement>('tr') ?? [])]
  }

  const contentIframeOrNull = (): HTMLIFrameElement | null => {
    return document.querySelector<HTMLIFrameElement>('#_content')
  }

  const clickedMenuTextOrNull = (): string | null => {
    const lastChild = document.body.querySelector('.jstree-clicked')?.lastChild
    return lastChild instanceof CharacterData ? lastChild.data : null
  }

  const dateOfTr = (tr: HTMLTableRowElement): Date | null => {
    return parseDateText(childElementText(tr, dateIndex()))
  }

  const significantLines = (tr: HTMLTableRowElement): string[] => {
    return (childElementText(tr, significantIndex()) ?? '')
      .split('\n')
      .map((line) => line.trim())
      .filter(Boolean)
  }
  ...
  // 창립기념일 7/2
  const foundationDay = (date = new Date()): Date => {
    date.setMonth(6, 2)
    date.setHours(0, 0, 0, 0)
    return date
  }

  const timeText = (h: number, m: number): string => {
  ...
  const detailTableOrNull = (): HTMLDivElement | null => {
    return document
      .querySelector<HTMLIFrameElement>('#_content')
      ?.contentDocument.querySelector<HTMLDivElement>('#grid2') ?? null
  }

  const summaryTableOrNull = (): HTMLDivElement | null => {
    return document
      .querySelector<HTMLIFrameElement>('#_content')
      ?.contentDocument.querySelector<HTMLDivElement>('#grid3') ?? null
  }
  ...
  const lackTimes = () => {
    ...
    const summaryTr = summaryTable?.querySelector('tbody')?.querySelector<HTMLTableRowElement>('tr') ?? null
    const lackTimeTxt = lackTimeIdx < 0
      ? ''
      : (summaryTr ? childElementText(summaryTr, lackTimeIdx) : '')
  ...
  // 오늘 출근한 시간
  const clockInTimeOfTodayOrNull = (): Date | null => {
    ...
    const clockInTimeText = todayTr ? childElementText(todayTr, clockInTimeIndex()) : ''
  ...
  const currentPageDate = (): Date => {
    const detailTable = detailTableOrNull()
    const bodyTrs = tableRows(detailTable)
    return dateOfTr(bodyTrs[0]) ?? new Date()
  }

  const parseMinutes = (mins: number): { hours: number, minutes: number } => {
    return { hours: Math.floor(mins / MINUTES_PER_HOUR), minutes: mins % MINUTES_PER_HOUR }
  }

  const myStatusLabel = (): string => {
  ...
  const clockOutTimeForLastDayOfMonth = (): { hours: number, minutes: number } | null => {
  ...
  const getTotalSavingMins = (): number => {
  ...
  const clockInTimeIndex = (): number => {
    return getDetailTableIndex('출근시간')
  }

  const workingDayTrs = (): HTMLTableRowElement[] => {
    const detailTable = detailTableOrNull()
    const bodyTrs = tableRows(detailTable)
    // 쉬는날은 빨간 글씨 부여를 위해 span으로 텍스트를 감싸서 firstElementChild가 있으므로 그게 없다면 근무일임
    return bodyTrs.filter((tr) => {
      const child = tr.childNodes[dateIndex()]
      return !(child instanceof Element && child.firstElementChild)
    })
  }

  const totalWorkingDaysCount = (): number => {
    return workingDayTrs().length
  }

  const remainingWorkingDayTrs = (): HTMLTableRowElement[] => {
  ...
  const remainingWorkingDaysCount = (): number => {
    return remainingWorkingDayTrs().length
  }

  const totalRemainingMins = (): number => {
    return remainingWorkingDaysCount() * WORKING_HOURS_PER_DAY * MINUTES_PER_HOUR
  }

  const usedWfhCount = (): number => {
    return workingDayTrs().filter(
      (tr) => childElementText(tr, significantIndex()) === '재택근무',
    ).length
  }

  const renderRow = ({ title, content }: { title: string, content: string }): string => {
  ...
  const buildPopover = (): void => {
    ...
    const closeBtn = popoverEl.getElementById(closeBtnId)
    if (closeBtn instanceof HTMLButtonElement) {
      closeBtn.addEventListener('click', clearOldPopover)
    }

    const firstChild = popoverEl.body.firstChild
    if (firstChild) {
      document.body.appendChild(firstChild)
    }
  }

  const openPopoverWhenReady = (): void => {
    const retry = 100
    let triedCount = 0
    ...
    const intervalId = setInterval(() => {
      ...
      const clickedMenu = clickedMenuTextOrNull()
      ...
      const loadings = [
        ...(document
          .querySelector<HTMLIFrameElement>('#_content')
          ?.contentDocument.querySelectorAll<HTMLElement>('.PUDD-UI-loading') ?? []),
      ]
      ...
      const searchBtn = document
        .querySelector<HTMLIFrameElement>('#_content')
        ?.contentDocument.querySelector<HTMLElement>('#searchBtn')
